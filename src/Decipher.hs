...
keyFromStr :: String -> Key
keyFromStr = keyFromList . (fmap (fromJust . convertLetter))
...
-- | This is the main decipher function.
decipher :: StdGen
         -> Dictionary
         -> Dictionary
         -> Int
...
decipher gen cutoff_dict evolve_dict key_len (pop_size, mutate_rate, percent_parents) text = do
  ...
  x <- evalStateT (evalStateT (evolve2 (mutate_rate, percent_parents) cutoff_dict evolve_dict text) xs) gen
  return x

{- |evolves the population until a solution is found
    a solution is found when a key deciphers a messassge
    in which all  words are contained in a given dictionary
-}
evolve :: (Percentage, Percentage)
       -> Dictionary -- ^ the complete dictionary to evaluate solution keys
       -> Dictionary -- ^ the partial dictionary to decode with
       -> EncryptedText
...
-- | generates a n random keys of length len
